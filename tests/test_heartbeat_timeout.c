...
void test_heartbeat_timeout_run(void)
{
    ...
    hb_init(&s, 0U);
    test_assert(s.heartbeat_timeout == 1U, "T1: boot state is timeout");
    test_assert(s.fail_safe == 1U,         "T1: boot state is fail-safe");

    /* T2: fresh heartbeat clears timeout */
    hb_receive(&s, 1U, 100U);
    test_assert(s.heartbeat_timeout == 0U, "T2: fresh HB clears timeout");
    test_assert(s.fail_safe == 0U,         "T2: fresh HB clears fail-safe");

    /* T3: same counter value does NOT refresh the timer */
    hb_receive(&s, 1U, 400U);
    hb_check_timeout(&s, 460U);
    test_assert(s.heartbeat_timeout == 1U, "T3: duplicate counter does not prevent timeout");
    ...
    test_assert(s.heartbeat_timeout == 0U, "T4: advancing counter within deadline keeps link alive");

    /* T5: timeout fires exactly after 350 ms gap */
    hb_init(&s, 0U);
    hb_receive(&s, 5U, 1000U);
    hb_check_timeout(&s, 1350U);
    test_assert(s.heartbeat_timeout == 0U, "T5: exactly at boundary — no timeout yet");
    hb_check_timeout(&s, 1351U);
    test_assert(s.heartbeat_timeout == 1U, "T5: one ms past boundary — timeout fires");

    /* T6: recovery — new counter after timeout restores normal state */
    hb_receive(&s, 6U, 2000U);
    test_assert(s.heartbeat_timeout == 0U, "T6: new counter after timeout restores normal");
    test_assert(s.fail_safe == 0U,         "T6: new counter after timeout clears fail-safe");

    /* T7: counter wraps 255 → 0 and is treated as a new value */
    hb_receive(&s, 255U, 3000U);
    hb_receive(&s, 0U,   3100U);
    test_assert(s.heartbeat_timeout == 0U, "T7: counter wrap 255->0 accepted as fresh HB");
}
